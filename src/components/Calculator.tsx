...
export function Calculator() {
  ...
  const estimatedPayback =
    localYearlySavings > 0 ? +(localCost / localYearlySavings).toFixed(1) : 0.0;
  ...
  const handleCalculate = async () => {
    ...
    try {
      ...
      try {
        ...
        const experienceYears = 5; // Default experience for demo

        const vendorRes = await api.recommendVendor(
          vendorPrice,
          vendorRating,
          experienceYears,
          location,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const message = e instanceof Error ? e.message : "Failed to save calculation";
      console.error("Save calculation failed", message, e);
      if (
          ...
          <div className="grid gap-8 lg:grid-cols-5">
              ...
              <div className="space-y-6">
                <div className="mb-6">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Get accurate solar recommendations
                  </p>
                </div>
            ...
            >
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <ResultCard
                  label="System Size"
                  value={sys?.recommended_capacity_kw ?? localSize}
                  suffix=" kW"
                  decimals={1}
                ...
                <ResultCard
                  label="Total Cost"
                  value={fin?.installation_cost_inr ?? localCost}
                  prefix="₹"
                  tint="var(--gradient-sky)"
                />
                <ResultCard
                  label="Yearly Savings"
                  value={fin?.annual_savings_inr ?? localYearlySavings}
                  prefix="₹"
                  tint="var(--gradient-eco)"
                />
                <ResultCard
                  label="Payback"
                  value={fin?.payback_period_years ?? estimatedPayback}
                  suffix=" yrs"
                  decimals={1}
                  tint="var(--gradient-solar)"
                />
              ...
              {result && (
                ...
                >
                  <div className="glass-premium rounded-2xl p-4 text-center">
                    ...
                    <div className="text-xl font-bold text-gradient-solar">
                      {sys?.panels_needed}
                    </div>
                  </div>
                  <div className="glass-premium rounded-2xl p-4 text-center">
                  ...
                  <div className="glass-premium rounded-2xl p-4 text-center">
                    ...
                    <div className="text-xl font-bold text-gradient-solar">
                      {result?.environment?.co2_offset_kg_per_year ?? 0} kg/yr
                    </div>
                  </div>
                </motion.div>
              )}
                    ...
                    <div className="glass-premium rounded-2xl p-4 border-l-4 border-solar">
                      ...
                      {vendorRec.vendor?.name === "No vendor available" ? (
                        <p className="text-sm text-muted-foreground break-words">
                          {vendorRec.recommendation}
                        </p>
                      ) : (
                        <>
                          <div className="text-sm font-semibold mb-2">
                            {vendorRec.vendor_score?.toFixed(1) || "0.0"}/100
                          </div>
                          <p className="text-xs text-muted-foreground mb-1">
                            ₹{vendorRec.vendor?.price_per_kw?.toLocaleString("en-IN")}/kW · Rating:{" "}
                            {vendorRec.vendor?.rating}/5
                          </p>
                          <p className="text-sm text-muted-foreground break-words">
                            {vendorRec.recommendation}
                          </p>
                          <div className="mt-3 text-xs bg-solar/10 rounded px-2 py-1 text-solar font-medium">
                            Confidence: {vendorRec.confidence}
                          </div>
                        </>
                      )}
                    </div>
                  ...
                  {schemeRec && (
                    <div className="glass-premium rounded-2xl p-4 border-l-4 border-green-500">
                      ...
                      <div className="text-lg font-bold text-green-500 mb-2">
                        {schemeRec.scheme?.name ??
                          schemeRec.recommended_scheme ??
                          "No scheme available"}
                      </div>
                      <p className="text-sm text-muted-foreground mb-1">
                        Subsidy: ₹
                        {(schemeRec.scheme?.subsidy ?? schemeRec.estimated_subsidy).toLocaleString(
                          "en-IN",
                        )}
                      </p>
                      <p className="text-xs text-muted-foreground mb-2">
                        Max: ₹{schemeRec.scheme?.max_amount?.toLocaleString("en-IN")}
                      </p>
                      <div className="text-xs bg-green-500/10 rounded px-2 py-1 text-green-600 font-medium">
                        Eligibility: {schemeRec.eligibility}
                      </div>
                    </div>
                  )}
                </motion.div>
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={chartData}>
                      ...
                      <YAxis
                        stroke="currentColor"
                        fontSize={12}
                        tickFormatter={(v) => `₹${Number(v ?? 0).toFixed(0)}k`}
                      />
                      <Tooltip
                        contentStyle={{
                          background: "var(--card)",
...
function ResultCard({
  ...
  return (
    ...
    >
      <motion.div
        className="absolute -top-8 -right-8 h-24 w-24 rounded-full opacity-0 blur-2xl group-hover:opacity-40 transition-opacity"
