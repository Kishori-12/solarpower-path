...
export function AdminAnalytics() {
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="glass-premium rounded-3xl h-64 animate-pulse" />
          ))}
        </div>
      </div>
    );
  ...
  const locationData = Object.entries(data.location_distribution).map(([name, value]) => ({
    name,
    value,
  }));
        ...
        {[
          {
            ...
            value: `₹${(data.overview.total_savings_inr / 1000).toFixed(0)}k`,
            sub: "across all calculations",
          },
          {
            label: "CO₂ Offset",
            value: `${(data.overview.total_co2_offset_kg / 1000).toFixed(1)}T`,
            sub: "tonnes of CO₂ saved",
          },
          {
            label: "Avg System Size",
            value: `${data.overview.avg_system_size_kw} kW`,
