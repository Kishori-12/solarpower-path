...
import { Users, Building2, Zap, FileText, IndianRupee, Leaf, TrendingUp, Clock } from "lucide-react";
...
export function AdminDashboard() {
  ...
  const stats = [
    { label: "Total Users",        value: ov?.total_users ?? 0,        icon: Users,       color: "text-blue-400",   bg: "bg-blue-400/10" },
    { label: "Total Vendors",      value: ov?.total_vendors ?? 0,      icon: Building2,   color: "text-purple-400", bg: "bg-purple-400/10" },
    { label: "Calculations",       value: ov?.total_calculations ?? 0, icon: Zap,         color: "text-solar-glow", bg: "bg-solar/10" },
    { label: "Active Schemes",     value: ov?.total_schemes ?? 0,      icon: FileText,    color: "text-green-400",  bg: "bg-green-400/10" },
    { label: "Total Savings (₹)",  value: ov?.total_savings_inr ?? 0,  icon: IndianRupee, color: "text-yellow-400", bg: "bg-yellow-400/10", format: "currency" },
    { label: "CO₂ Offset (kg)",    value: ov?.total_co2_offset_kg ?? 0,icon: Leaf,        color: "text-emerald-400",bg: "bg-emerald-400/10" },
    { label: "Avg System (kW)",    value: ov?.avg_system_size_kw ?? 0, icon: TrendingUp,  color: "text-orange-400", bg: "bg-orange-400/10", decimal: true },
    { label: "Pending Vendors",    value: vs?.pending ?? 0,            icon: Clock,       color: "text-red-400",    bg: "bg-red-400/10" },
  ];
          ...
          <div className="flex flex-wrap gap-3">
            {Object.entries(data.location_distribution).map(([loc, count]) => (
              <div key={loc} className="glass-premium rounded-xl px-4 py-2 text-sm">
                ...
                <span className="text-muted-foreground ml-2">{count} vendor{count !== 1 ? "s" : ""}</span>
              </div>
            ))}
          </div>
        </motion.div>
