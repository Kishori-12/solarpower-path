...
export function AdminSchemes() {
  ...
  const openEdit = (s: AdminScheme) => {
    setForm({
      ...
      max_subsidy_inr: s.max_subsidy_inr,
  ...
  const handleSave = async () => {
    if (!form.name || !form.provider || !form.eligibility) {
      setError("Name, provider, and eligibility are required");
      return;
    }
    setSaving(true);
    setError("");
  ...
  const inputCls =
    "w-full rounded-xl bg-input/60 border border-border/50 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-solar transition-all";
            ...
            >
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 py-2.5 rounded-xl glass-premium text-sm font-semibold"
                >
                  Cancel
                </button>
                <button
        ...
        {showForm && (
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <input
                type="number"
                placeholder="Max Subsidy (₹)"
                value={form.max_subsidy_inr}
                onChange={(e) => set("max_subsidy_inr", +e.target.value)}
                min={0}
                className={inputCls}
              />
              <input
            ...
            {error && (
              <p className="text-sm text-red-400 bg-red-400/10 rounded-lg px-3 py-2 mt-3">
                {error}
              </p>
            )}
      ...
      {loading ? (
        <div className="space-y-3">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="glass-premium rounded-2xl h-24 animate-pulse" />
          ))}
        </div>
      ) : schemes.length === 0 ? (
        ...
        <div className="space-y-3">
          {schemes.map((s, i) => (
            ...
            >
              <div
                className={`h-10 w-10 rounded-xl flex items-center justify-center shrink-0 ${s.active ? "bg-green-400/10" : "bg-muted/30"}`}
              >
                <FileText
                  className={`h-5 w-5 ${s.active ? "text-green-400" : "text-muted-foreground"}`}
                />
              </div>
              <div className="flex-1 min-w-0">
                ...
                <div className="text-sm text-muted-foreground">
                  {s.provider} · {s.subsidy_percent}% subsidy · up to ₹
                  {s.max_subsidy_inr.toLocaleString("en-IN")}
                </div>
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg bg-red-400/10 hover:bg-red-400/20 transition-all text-red-400"
                >
