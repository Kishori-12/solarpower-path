    ...
    <div>
      ...
      {loading ? (
        <div className="space-y-2">
          {Array.from({ length: 5 }).map((_, i) => <div key={i} className="glass-premium rounded-xl h-16 animate-pulse" />)}
        </div>
      ) : filtered.length === 0 ? (
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border/30">
                <th className="text-left px-5 py-3 text-xs uppercase tracking-wider text-muted-foreground font-semibold">User</th>
            ...
            <tbody>
              {filtered.map((u, i) => (
                ...
                >
                  ...
                  <td className="px-5 py-3 text-sm text-muted-foreground">{u.created_at?.split("T")[0]}</td>
                </motion.tr>
              ))}
            </tbody>
          </table>
