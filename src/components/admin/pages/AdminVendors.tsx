...
export function AdminVendors() {
  ...
  const handleReject = async () => {
    ...
    try {
      await adminApi.rejectVendor(rejectTarget, rejectReason || "Does not meet platform requirements");
    ...
    <div>
      ...
      {loading ? (
        <div className="space-y-3">
          {Array.from({ length: 4 }).map((_, i) => <div key={i} className="glass-premium rounded-2xl h-20 animate-pulse" />)}
        </div>
      ) : filtered.length === 0 ? (
              ...
              >
                ...
                <div className="flex items-center gap-4 p-5">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-sm text-muted-foreground">{v.email} · {v.location} · ₹{v.price_per_kw.toLocaleString("en-IN")}/kW</div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    {status === "pending" && (
                      <>
                        ...
                        >
                          {actionLoading === v.id ? <Loader2 className="h-3 w-3 animate-spin" /> : <CheckCircle className="h-3 w-3" />}
                          Approve
                        </motion.button>
                        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                ...
                <AnimatePresence>
                  {isExpanded && (
                    ...
                    >
                      <div className="text-xs uppercase tracking-wider text-muted-foreground font-semibold mb-3">Uploaded Documents</div>
