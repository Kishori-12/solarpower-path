...
export function VendorAuthModal({ open, onClose, initialTab = "login" }: Props) {
  ...
  const inputCls =
    "w-full rounded-xl bg-input/60 border border-border/50 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-solar transition-all";
              ...
              {tab === "register" && (
                <>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <input
                      type="number"
                      placeholder="Price/kW (₹)"
                      value={form.price_per_kw}
                      onChange={(e) => set("price_per_kw", e.target.value)}
                      required
                      min={1}
                      className={inputCls}
                    />
